...
继续任务前读取原生状态、检查点和最新回执。最高研究接受阶段不是当前工作阶段；已关闭或被拒绝的任务不得通过更大的阶段编号重新启动。

`validate_project_state_transition.py` 现在核对当前任务身份、阶段和关闭状态。五个历史一次性工具在读取旧载荷、写入文件之前调用 `require_current_action`，检查原生状态与检查点绑定以及实际动作/参数；自动 push 触发已移除。新任务必须先建立自己的原生合同和允许动作。

覆盖范围是这些已接入的脚本，不是对所有聊天工具的全局拦截。可运行 `python -m unittest discover -s tests -p 'test_project_state*.py' -v` 复验旧任务拒绝、合法当前动作以及写入前拒绝。
