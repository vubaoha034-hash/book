...
- **微观层**：对白为什么此刻发生、信息为什么由这个人说给这个人、相对时间/钱/物件/知识等语义作用域是否跨句连续。
...
另外建立 `04b-local-semantic-logic-ledger.md`。不能只记录“昨晚发生 A、前天发生 B”，还要记录 A 是否通过列表、解释、代词、花销、物件转移等关系把时间/状态作用域传给 B。
...
不要给每句对白硬配动作。没有可信触发时，应删句、换信息载体、允许沉默/延迟/打断，或重做交流目的。
...
正文生成只携带当前场景的**最小 microcraft 指令**：人物当前目标、必要 trigger、listener knowledge、信息载体与允许的 non-speech 选择。禁止把整套审稿清单塞进正文上下文。

三条正文纪律：

1. 不为了交代背景，让人物说给一个不需要听的人；
2. 不为了“自然”机械插入看、叹气、摸东西、皱眉；
3. 不为了“有趣”连续搭建问答—反问—追问—包袱，而不改变场内状态。
...
Fresh context 只代表历史信息隔离，不代表它天然具备更强的逐句逻辑检测能力。局部语义与对白真值必须另跑 G6D。
...
- speaker 的场内目标；
- listener 是否已知；
- 如果没有读者，speaker 是否仍会说；
- 该信息是否有合法场内 carrier；
- 沉默、延迟、截断、答非所问或行动是否更可信；
- 对话后知识/选择/关系/风险/任务是否改变；
- 时间词是否通过列表、花销、代词、解释等语义关系继承作用域；
- 是否用通用动作 beat 假装修复 grounding。
...
G6D 修复顺序优先：删掉不需要的话 → 把信息移到真实触发点 → 改信息载体/说话目的 → 使用可信的 non-speech 选择 → 最后才改措辞。禁止靠动作 beat 堆砌“真人感”。

## Phase 10：个人审美与去 AI

只有 G1、G6、**G6D** 均通过、场景顺序稳定后，才读取 `modules/aesthetic-fingerprint.md` 与行文模块。

先识别当前稿相对认可/拒绝样本最明显的二至五个偏离，再做最小修改。样本为空则只执行审美基线并说明限制。

去 AI 不能替代 G6D：一个“说得很好听”的作者嘴替仍然是 FAIL；一个加了动作的无动机对白仍然是 FAIL。
...
- 主要情绪债得到真实偿还或记录为有意风险。
- 修改位置没有产生连锁冲突。
- `V3-G6D = PASS`，且每个 PASS 有定位证据，不是“已读过/感觉自然”。
- 不存在已知的时间语义冲突、作者嘴替式关键对白或无动机 ping-pong 模板。
