...
- 强标题：整行的 `第N章`、`第N回`、`Chapter N` 及带合法分隔符的短标题形式。
- 序列型紧凑回目：额外扫描位于物理行尾的 `第N回短标题`。短标题必须为 2–12 个非空白字符，不得包含 `，,。！？!?；;…`；只有形成至少 3 个连续编号的候选链才启用。候选可以独占一行，也可以紧接上一段正文，分割点始终位于“第”字之前。
...
通过章节序列 Hard Gate 的紧凑 `第N回短标题` 会沿用同一机械编号提取；这不改变 Source Integrity Gate 的章节占比、缺口、重复、倒退或系统性缺章阈值。

## 章节策略

引擎先做整行标题候选检测，并独立扫描通过连续编号 Hard Gate 的紧凑行尾回目，再处理卷标记与章标题关系，最后检查候选位置和分段长度。它支持“第一章”“第1章”“第一回”“第十二回”“Chapter 1”“卷一”“第一卷”“序章”“楔子”“番外”“后记”等形式。
